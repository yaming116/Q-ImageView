 ...
 */
public class QImageMogr2UrlBuilder {
    ...
    int rotate = QImage.ORIGINAL_QUALITY;
    int frameNumber = QImage.ORIGINAL_QUALITY;
    int width = QImage.ORIGINAL_QUALITY;
    int height = QImage.ORIGINAL_QUALITY;
    int dx = QImage.ORIGINAL_QUALITY;
    int dy = QImage.ORIGINAL_QUALITY;
    ...
    public QImageMogr2UrlBuilder frameNumber(int frameNumber){
        if (frameNumber < 1 || frameNumber >= 100) {
            throw new IllegalArgumentException("FrameNumber must be between 1 and 101, inclusive.");
        }
